...
export function ProfileForm({ canCancel, error, form, isSetupMode, onCancel, onChange, onSubmit, saving }: ProfileFormProps) {
  return (
    <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
      <aside className="space-y-5 lg:pt-8">
        <PageHeading
          title={isSetupMode ? "Set up your practice profile" : "Edit your practice profile"}
        ...
        <div className="rounded-[1.75rem] border border-violet-200/70 bg-white/90 p-6 shadow-[0_24px_55px_-40px_rgba(76,29,149,0.35)] backdrop-blur">
          <p className="font-mono text-sm text-slate-500">Editing setup</p>
          <p className="mt-4 text-2xl font-semibold tracking-tight text-[#171326]">{form.nickname || "Email name fallback"}</p>
      ...
      <SurfaceCard>
        <form className="space-y-5" onSubmit={onSubmit}>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            ...
            {canCancel ? (
              <AppButton disabled={saving} type="button" variant="secondary" onClick={onCancel}>
                Cancel
              </AppButton>
            ) : null}
          </div>
