...
export function CurrentSessionTasks({
  ...
  return (
    <SurfaceCard className="rounded-[2rem] border-violet-200/70 p-6 shadow-[0_24px_60px_-42px_rgba(76,29,149,0.28)] sm:p-8">
        ...
        {session.tasks.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <label className="mt-3 flex cursor-pointer items-center justify-between gap-4 rounded-2xl border border-violet-200/70 bg-violet-50 px-4 py-3 text-sm font-medium text-violet-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.85)] transition-[background-color,border-color,transform] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:border-violet-300 hover:bg-white active:scale-[0.98]">
                <span>{checked ? "Completed" : "Mark as completed"}</span>
                <input
                  type="checkbox"
                  checked={checked}
                  onChange={() => onToggleTask(item.taskId)}
                  className="h-4 w-4 rounded border-violet-300 text-violet-700 focus:ring-2 focus:ring-violet-700/20"
                />
              </label>
            </li>
          );
      ...
      <div className="music-accent-panel mt-6 rounded-[1.5rem] border border-violet-950/70 p-5 shadow-[0_18px_42px_-34px_rgba(46,16,101,0.65)]">
